...
mut.to.sigs.input <- function(mut.ref, sample.id = "Sample", chr = "chr", pos = "pos", ref = "ref", alt = "alt", bsg = NULL) {
  ...
  # Only take SNPs for now (single-base ref/alt)
  mut <- mut[which(
    mut[, ref] %in% c("A", "T", "C", "G") &
      mut[, alt] %in% c("A", "T", "C", "G")
  ), ]
  ...
  # If set to default (hg19), carry on as original
  if (is.null(bsg)) {
    # Remove any entry in chromosomes that do not exist in the
    # BSgenome.Hsapiens.UCSC.hg19::Hsapiens object
    unknown.regions <- levels(mut[, chr])[which(
      !(levels(mut[, chr]) %in%
          GenomeInfoDb::seqnames(BSgenome.Hsapiens.UCSC.hg19::Hsapiens))
    )]
    if (length(unknown.regions) > 0) {
      unknown.regions <- paste(unknown.regions, collapse = ", ")
      warning(paste(
        "Check chr names -- not all match BSgenome.Hsapiens.UCSC.hg19::Hsapiens object:\n",
        unknown.regions,
        sep = " "
      ))
      mut <- mut[
        mut[, chr] %in% GenomeInfoDb::seqnames(BSgenome.Hsapiens.UCSC.hg19::Hsapiens),
      ]
    }
    # Add in context
  ...
  if (!is.null(bsg)) {
    ...
    # Remove any entry in chromosomes that do not exist in the BSgenome object
    unknown.regions <- levels(mut[, chr])[which(
      !(levels(mut[, chr]) %in% GenomeInfoDb::seqnames(bsg))
    )]
    if (length(unknown.regions) > 0) {
      unknown.regions <- paste(unknown.regions, collapse = ", ")
      warning(paste(
        "Check chr names -- not all match", bsg,
        "object:\n", unknown.regions,
        sep = " "
      ))
  ...
  if (any(substr(mut[, ref], 1, 1) != substr(mut[, "context"], 2, 2))) {
    bad <- mut[
      which(substr(mut[, ref], 1, 1) != substr(mut[, "context"], 2, 2)),
    ]
    bad <- paste(
      bad[, sample.id],
      bad[, chr],
  ...
  mut$std.mutcat[c(gind, tind)] <- gsub(
    "G", "g",
    gsub("C", "c",
         gsub("T", "t",
              gsub("A", "a", mut$std.mutcat[c(gind, tind)])))
  ) # to lowercase
  mut$std.mutcat[c(gind, tind)] <- gsub(
    "g", "C",
    gsub("c", "G",
         gsub("t", "A",
              gsub("a", "T", mut$std.mutcat[c(gind, tind)])))
  ) # complement

  mut$std.context <- mut$context
  mut$std.context[c(gind, tind)] <- gsub(
    "G", "g",
    gsub("C", "c",
         gsub("T", "t",
              gsub("A", "a", mut$std.context[c(gind, tind)])))
  ) # to lowercase
  mut$std.context[c(gind, tind)] <- gsub(
    "g", "C",
    gsub("c", "G",
         gsub("t", "A",
              gsub("a", "T", mut$std.context[c(gind, tind)])))
  ) # complement
  mut$std.context[c(gind, tind)] <- sapply(
    strsplit(mut$std.context[c(gind, tind)], split = ""),
  ...
  final.matrix <- matrix(
    0,
    ncol = 96,
    nrow = length(unique(mut[, sample.id]))
  )
